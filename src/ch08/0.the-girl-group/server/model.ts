...
export function getIdols(params = <any>{}): Idol[] {
  let result = idols;

  if (params.name) {
    result = result.filter(i => i.name.indexOf(params.name) !== -1);
  }
  if (parseInt(params.height) && result.length > 0) {
    result = result.filter(i => i.height > parseInt(params.height));
  }
  if (params.category && params.category > 0 && result.length > 0) {
    result = result.filter(i => i.categories.indexOf(params.category) !== -1);
  }

  return result;
}

export function getIdolById(idolId: number): Idol {
  return idols.find(i => i.id === idolId);
}

export function getReviewsByIdolId(idolId: number): Review[] {
